...
def train(dataset_path, train_conf, model_path, add_timestamp):
    ...
    logger.info(dataset)
    print("run test", dataset.get_num_labels())
    ...
    train_history = model.fit(dataset.get_train_sequence(), epochs=train_conf["epochs"],
    ...
    os.chdir("/Users/boumahrat/Desktop/EPF/5A/from_poc_to_prod/poc-to-prod-capstone/train/data/artefacts")
    ...
    with open(f'./{artefacts_path}/params.json', 'w') as json_file:
        json.dump(train_conf, json_file)


    # TODO: CODE HERE
    # save labels index in artefacts_path/labels_index.json
    file_label = dataset.get_index_to_label_map()

    # Serializing json
    json_object = json.dumps(file_label, indent=4)

    # Writing to sample.json
    with open(f'./{artefacts_path}/labels_index.json', "w") as outfile:
    ...
    with open(f'./{artefacts_path}/train_output.json', "w") as f:
